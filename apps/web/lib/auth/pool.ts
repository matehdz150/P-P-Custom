/**
 * Entrar a un pool de Cognito, sin SDK.
 *
 ...
 *
 * ES UNA FÁBRICA Y NO UN MÓDULO SUELTO porque hay TRES pools —talleres,
 * compradores y el backoffice— y el flujo es el mismo en los tres: entrar,
 * responder el reto de contraseña temporal, renovar. Escrito una vez por pool,
 * lo que se separa con el tiempo es justo lo que no debe: el manejo de la
 * sesión. Cada pool sólo aporta su id de cliente y dónde guarda sus tokens.
 *
 * CADA POOL CON SU LLAVE DE ALMACENAMIENTO, y no es un detalle: compartirla
 * dejaría que entrar al backoffice cerrara la sesión del taller en la misma
 * máquina, o peor, que una pantalla mandara el token del pool equivocado y
 * recibiera un 401 que no se explica.
 *
 ...
 *   robar. Pero una cookie httpOnly la pone un servidor, y el front es
 *   estático en S3 + CloudFront: no hay servidor que la ponga.
 *
 *   Así que el trato es explícito: si alguien logra ejecutar JavaScript en
 *   nuestro dominio, se lleva la sesión. Se acota con tokens cortos (1 hora) y,
 *   en el backoffice, con un refresco de 1 día en vez de 30.
 */
...
/**
 * Ata el flujo a un pool concreto.
 *
 * `cliente` es el id del app client de ese pool y `llave` dónde guarda sus
 * tokens en el navegador. Nada más cambia entre uno y otro.
 */
export function crearAuth({
	 ...
	 * se pisarían al guardar. Es por pool, no global.
	 */
	...
	async function entrar(email: string, password: string) {
		if (!CLIENTE)
			throw new ErrorCognito("Config", "Falta NEXT_PUBLIC_COGNITO_CLIENTE");
	...
	async function establecerContrasena(reto: CambioDeContrasena, nueva: string) {
