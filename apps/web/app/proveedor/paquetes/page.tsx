...
import { getMyPackages, type ProviderPackage } from "@/lib/api/providers";
	...
	return (
		<div className="min-h-screen bg-white px-6 py-6 sm:px-8">
			...
			<div className="mb-6 flex items-center justify-between">
				<div>
					<h1 className="text-[22px] font-bold text-[#1a1a1a]">Mis paquetes</h1>
					{!loading && (
			...
			{!loading && packages.length === 0 && (
				<div className="flex flex-col items-center justify-center py-24 text-center">
					...
					<p className="mt-4 text-[15px] font-semibold text-[#1a1a1a]">Todavía no tienes paquetes</p>
					<p className="mt-1.5 max-w-xs text-[13px] text-[#888]">
						Agrupa tus productos en paquetes para ofrecerlos a un mejor precio.
					</p>
					<Link
						href="/proveedor/paquetes/nuevo"
...
function PackageCard({ pkg }: { pkg: ProviderPackage }) {
	...
	return (
		<article className="group relative">
			...
			<div className="mt-2.5 px-0.5">
				<p className="truncate text-[13px] font-semibold text-[#1a1a1a]">{pkg.name}</p>
				<p className="mt-0.5 text-[12px] font-semibold text-[#1a1a1a]">
					{pkg.pricing ? `$${pkg.pricing.basePrice}` : "—"}
				</p>
			</div>
		</article>
	);
