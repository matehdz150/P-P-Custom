...
import { Check, ChevronLeft, ChevronRight, Minus, Package, Plus, X } from "lucide-react";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { createProviderPackage, getMyProducts, type ProviderProduct } from "@/lib/api/providers";
...
export default function NewProviderPackagePage() {
	const router = useRouter();
	const [step, setStep] = useState(0);
	const [saving, setSaving] = useState(false);

	/* step 1 */
	const [form, setForm] = useState<FormData>({ name: "", description: "", basePrice: "" });
	...
	async function handleSubmit() {
		...
		try {
			await createProviderPackage({
				...
				items: selected.map((s) => ({ productId: s.product.id, quantity: s.quantity })),
			});
	...
	return (
		<div className="min-h-screen bg-[#f7f7f5]">
					...
					<div className="flex items-center gap-2">
						{STEPS.map((label, i) => (
							<div key={label} className="flex items-center gap-2">
								...
								{i < STEPS.length - 1 && (
									<div className={`h-px w-8 ${i < step ? "bg-[#171717]" : "bg-[#d9d9d2]"}`} />
								)}
							</div>
						))}
					</div>
			...
			<div className="mx-auto max-w-3xl px-4 py-8">
				...
				{step === 2 && (
					<StepReview form={form} selected={selected} onRemove={(id) => setSelected((p) => p.filter((s) => s.product.id !== id))} />
				)}
				...
				<div className="mt-8 flex items-center justify-between">
					...
					{step < STEPS.length - 1 ? (
						...
						>
							{saving ? "Guardando…" : "Publicar paquete"}
						</button>
					)}
				</div>
			</div>
...
function StepInfo({ form, onChange }: { form: FormData; onChange: (f: FormData) => void }) {
	return (
		<div className="space-y-6">
			<div>
				<h2 className="text-2xl font-bold text-[#171717]">Información del paquete</h2>
				<p className="mt-1 text-sm text-[#77776f]">Dale nombre, descripción y precio a tu paquete.</p>
			</div>

			<div className="rounded-2xl border border-[#e7e7e2] bg-white p-6 space-y-5">
				<div className="space-y-1.5">
					<label className="text-sm font-semibold text-[#171717]">Nombre del paquete *</label>
					<input
						type="text"
				...
				<div className="space-y-1.5">
					<label className="text-sm font-semibold text-[#171717]">Descripción</label>
					<textarea
				...
				<div className="space-y-1.5">
					<label className="text-sm font-semibold text-[#171717]">Precio base *</label>
					<div className="relative">
						<span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-[#77776f]">$</span>
						<input
							type="number"
							min="0"
							step="0.01"
	...
	return (
		<div className="space-y-6">
			<div className="flex items-end justify-between">
				<div>
					<h2 className="text-2xl font-bold text-[#171717]">Selecciona los productos</h2>
					<p className="mt-1 text-sm text-[#77776f]">Elige uno o más productos de tu catálogo para incluir en el paquete.</p>
				</div>
				{selected.length > 0 && (
			...
			{loading ? (
				<div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
					{Array.from({ length: 6 }).map((_, i) => (
						<div key={i} className="animate-pulse rounded-2xl border border-[#e7e7e2] bg-white">
				...
				<div className="rounded-2xl border-2 border-dashed border-[#d9d9d2] bg-white py-16 text-center">
					...
					<p className="mt-3 text-sm font-semibold text-[#171717]">No tienes productos todavía</p>
					<p className="mt-1 text-sm text-[#77776f]">Primero crea productos para poder armar paquetes.</p>
				</div>
			) : (
				<div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
					{products.map((product) => {
						...
						return (
							...
							>
								...
								{/* info */}
								<div className="p-3">
									<p className="truncate text-sm font-semibold text-[#171717]">{product.name}</p>
									<p className="mt-0.5 text-xs text-[#77776f]">
										{product.pricing ? `$${product.pricing.basePrice}` : "Sin precio"}
									</p>
	...
	return (
		<div className="space-y-6">
			<div>
				...
				<p className="mt-1 text-sm text-[#77776f]">Confirma los detalles antes de publicar.</p>
			</div>
			...
			<div className="rounded-2xl border border-[#e7e7e2] bg-white p-6 space-y-3">
				<div className="flex items-start justify-between">
					<div>
						<p className="text-xs font-semibold uppercase tracking-wide text-[#aaa]">Nombre</p>
						<p className="mt-0.5 text-lg font-bold text-[#171717]">{form.name}</p>
					</div>
					<div className="text-right">
						<p className="text-xs font-semibold uppercase tracking-wide text-[#aaa]">Precio</p>
						<p className="mt-0.5 text-lg font-bold text-[#171717]">${Number(form.basePrice).toFixed(2)}</p>
					</div>
				</div>

				{form.description && (
					<p className="text-sm text-[#77776f] leading-relaxed border-t border-[#f0f0ec] pt-3">{form.description}</p>
				)}
			</div>
			...
			<div className="rounded-2xl border border-[#e7e7e2] bg-white divide-y divide-[#f0f0ec]">
				<div className="px-5 py-3">
					<p className="text-sm font-semibold text-[#171717]">
						{selected.length} producto{selected.length > 1 ? "s" : ""} incluido{selected.length > 1 ? "s" : ""}
					</p>
				</div>
				{selected.map((item) => {
					...
					return (
						<div key={item.product.id} className="flex items-center gap-4 px-5 py-4">
							...
							<div className="flex-1 min-w-0">
								<p className="truncate text-sm font-semibold text-[#171717]">{item.product.name}</p>
								<p className="text-xs text-[#77776f]">
									{item.product.pricing ? `$${item.product.pricing.basePrice}` : "Sin precio"} · cantidad: {item.quantity}
								</p>
							</div>
				...
				{total > 0 && (
					<div className="flex items-center justify-between px-5 py-3 bg-[#f7f7f5] rounded-b-2xl">
						...
						<p className="text-sm font-semibold text-[#171717]">${total.toFixed(2)}</p>
					</div>
				)}
			</div>
		</div>
